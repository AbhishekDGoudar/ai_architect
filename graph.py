...
def visuals_node(state: AgentState):
    # 1. Initialize the LLM and Meter
    llm = get_llm(state['provider'], state['api_key'], "smart")
    meter = TokenMeter()
    
    # 2. Call the AI Agent (Visual Architect)
    # This uses the specific prompt in agents.py to generate rich Mermaid syntax
    diagram_spec = agents.visual_architect(state['hld'], llm, meter)
    
    # 3. Save files for reference (optional, as UI renders from memory)
    paths = []
    if diagram_spec.system_context:
        paths.append(tools.run_diagram_code(diagram_spec.system_context, "system_context"))
    if diagram_spec.container_diagram:
        paths.append(tools.run_diagram_code(diagram_spec.container_diagram, "container_diagram"))
    if diagram_spec.data_flow:
        paths.append(tools.run_diagram_code(diagram_spec.data_flow, "data_flow"))
    
    # 4. Return the result to the state
    return {
        "diagram_code": diagram_spec,
        "diagram_path": paths,
        "total_tokens": state.get("total_tokens", 0) + meter.total_tokens,
        "logs": [{"role": "Visuals", "message": "Diagrams generated by AI"}]
    }
